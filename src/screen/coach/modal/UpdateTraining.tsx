...
import {add_training, update_training} from '../../../redux/feature/featuresSlice';
const UpdateTraining = ({visible, onClose, data}) => {
...
useEffect(()=>{
    setTtype(data.training_type)
setName(data.name)
    setDate(new Date(data?.training_date))
    setTime(new Date(data?.training_time))
    setTDuration(new Date(data?.training_duration))
    setLocation(data?.training_location)
    setDiscription(data?.training_description)
},[data])
  useEffect(() => {
    if (visible) {
      openModal();
  ...
  const Add_Training = async() => {
    ...
    onClose();
   await dispatch(update_training(params));
  };
  return (
